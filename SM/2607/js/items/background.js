...
preloadBackgroundImage(sceneId) {
    return new Promise((resolve) => {
        ...
        img.onerror = () => resolve();
        img.src = src;
        setTimeout(resolve, 3000); // таймаут на случай ошибки
    });
},
...
setMode(mode) {
    ...
    _loadImage() {
        if (this._currentMode === 'menu') return; // у меню картинка уже загружена
...
_applyTranslate() {
    ...
    _bindDragEvents() {
        ...
        document.addEventListener('touchstart', (e) => this._startDrag(e), { passive: false });
    ...
    this._clampTranslate();   // ← теперь центрирование/ограничение применяется единообразно
    this._applyTranslate();
    e.preventDefault();
},
