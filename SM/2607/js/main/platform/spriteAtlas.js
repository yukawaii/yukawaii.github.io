...
const SpriteAtlas = {
    ...
    _loaded: false,
    _callbacks: [],
    _spriteMap: {},   // карта для мгновенного доступа
    ...
    loadAll(callback) {
        ...
        const atlasGroups = {
            ...
            // ★★★ ДОБАВЛЯЕМ АТЛАС ДЛЯ ИВЕНТОВ ★★★
                events: [
                    { path: 'images/atlas/event_atlas0.png', dataPath: 'images/atlas/event_atlas0.json' }
                ]
        };
...
getSprite(atlasName, spriteName) {
    // Если spriteName уже содержит слеш, ищем как есть, иначе добавляем группу
    const key = spriteName.includes('/') ? spriteName : `${atlasName}/${spriteName}`;
    return this._spriteMap[key] || null;
},
...
_buildSpriteMap() {
    this._spriteMap = {};
    for (const [group, parts] of Object.entries(this._atlases)) {
        for (const part of parts) {
            if (!part.data || !part.data.frames) continue;
            for (const [name, frame] of Object.entries(part.data.frames)) {
                // Используем имя как есть, оно уже включает путь, например "ui/ramki/ramka1.png"
                this._spriteMap[name] = {
                    image: part.image,
                    sx: frame.frame.x,
                    sy: frame.frame.y,
                    sw: frame.frame.w,
