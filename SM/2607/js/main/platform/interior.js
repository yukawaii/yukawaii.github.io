    ...
    return new Promise((resolve) => {
        ...
        ids.forEach(id => {
            ...
            if (data && data.imagePath) {
                // Определяем реальный URL (сначала пробуем атлас)
                const spriteName = `interiors/${id}.png`;
                const dataUrl = SpriteAtlas.getSpriteDataURL('interiors', spriteName);
                const src = dataUrl || data.imagePath;
                // Добавляем загрузку с кешированием
                promises.push(this._loadImageCached(src));
            }
        });
        ...
        Promise.all(promises)
            .then(() => resolve())
            .catch(() => resolve()) // в любом случае завершаем
            .finally(() => {
                // Запасной таймаут на случай, если какие-то промисы зависнут
...
/**
 * Загружает изображение с кешированием по URL.
...
addInterior(interiorDataOrId) {
    ...
    const dataUrl = SpriteAtlas.getSpriteDataURL('interiors', spriteName);
    const src = dataUrl || interiorData.imagePath;

    // --- Загружаем с кешированием ---
    this._loadImageCached(src)
        .then((loadedImg) => {
            // Создаём элемент и копируем загруженное изображение
            const img = document.createElement('img');
            img.className = 'interior-item';
            img.dataset.interiorId = interiorId;
            img.src = loadedImg.src;  // используем уже загруженный объект
            ...
            // Если изображение уже полностью загружено – позиционируем сразу
            if (loadedImg.complete && loadedImg.naturalWidth > 0) {
                positionInterior();
            } else {
                // Иначе ждём загрузки (хотя она уже выполнена, но на всякий случай)
                img.onload = positionInterior;
                img.onerror = () => console.warn('[InteriorManager] Ошибка отображения:', img.src);
            }
        ...
        .catch((err) => {
            console.warn('[InteriorManager] Ошибка загрузки интерьера:', err);
        });
},
...
saveInterior(sceneId, interiorId, state = true) {
    const interiors = Storage.getInteriorsState() || {};
    if (!interiors[sceneId]) interiors[sceneId] = {};
    interiors[sceneId][interiorId] = state;
    Storage.saveInteriorsState(interiors);
},

onSceneChange(sceneId) {
      this._currentSceneId = sceneId;
    const interiors = Storage.getInteriorsState() || {};
    const sceneState = interiors[sceneId] || {};
...
addInteriorById(interiorData, sceneId) {
    ...
    // ---- Получение состояния всех интерьеров для сохранения ----
    getStates() {
        return Storage.getInteriorsState() || {};
    },
