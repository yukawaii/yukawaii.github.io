...
document.getElementById('menu-reset-btn')?.addEventListener('pointerdown', (e) => {
    ...
    if (confirm(getText('menu_reset_confirm', 'Сбросить весь прогресс?'))) {
        ...
        SceneManager.show('menu');
        const menuScore = document.getElementById('menu-score-display');
        if (menuScore) menuScore.innerHTML = `<img src="${this.pointsUrl}" style="width:1.5em;height:1.5em;vertical-align:middle;"> 0`;
...
// ивент
                document.getElementById('event-btn')?.addEventListener('pointerdown', (e) => {
                    if (e.button !== 0) return;
                    if (EventManager._activeEvent) {
                        EventManager.handleEventButtonClick();
                    }
                });
