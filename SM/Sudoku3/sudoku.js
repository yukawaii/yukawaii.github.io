    ...
    showRewardedAd() {
     //   console.log('showRewardedAd вызван');
        ...
        return new Promise((resolve) => {
            ...
            .then((result) => {
    clearTimeout(timeoutId);
    // Логируем ответ для отладки (можно убрать в продакшене)
    console.log('VKWebAppShowNativeAds result:', result);

    if (result) {
        // Прямые поля
        if (result.result === true) { resolve(true); return; }
        if (result.success === true) { resolve(true); return; }
        if (result.status === 'success') { resolve(true); return; }

        // Вложенные поля (data.result)
        if (result.data) {
            if (result.data.result === true) { resolve(true); return; }
            if (result.data.success === true) { resolve(true); return; }
        }

        // Если нет явной ошибки, но есть какой-то результат – считаем успехом
