...
BasicGame.Level.prototype = {
    ...
    update: function () {
        ...
        if (!this.game.playerHasMoved && this.timer > 1 && this.message == null) {
            myadd1();
            if (this.onMobile) {
                this.message = new Message(this.game, 'YpOBEHR '+(this.id+1), '  HAGMz HA kKpAH z DEpGz DLS 3AVEPA\n\n BODz PALRVEM DLS CTpELRln', false);
            } else {
                this.message = new Message(this.game, 'YpOBEHR '+(this.id+1), ' HAGMz PpOlEL z DEpGz DLS 3AVEPA\n\n KLzKAJ DLS CTpELRln', false);
            }
        ...
        if (!this.game.player.playerAlive) {
            if (this.message == null) {
                myadd1();
                if (this.onMobile) {
                    this.message = new Message(this.game, 'PpOBAL :(', 'HAGMz HA kKpAH', true);
                } else {
                    this.message = new Message(this.game, 'PpOBAL :(', 'HAGMz PpOlEL ', true);
                }
            }
        ...
        if (this.game.playerWins) {
            if (this.id < 99) {
                if (this.message == null) {
                    if (this.onMobile) {
                        this.message = new Message(this.game, 'POlEDA!:)', 'HAGMz HA kKpAH', true);
                    } else {
                        this.message = new Message(this.game, 'POlEDA! :)', 'HAGMz PpOlEL ', true);
                    }
                ...
                if (this.message == null) {
                    this.message = new Message(this.game, 'YPA!!', 'BCE YPOBHz PpOJDEHn!!!', true);
