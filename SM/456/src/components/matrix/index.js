...
import { getCurrentMode, GAME_MODES } from '../../unit/modes';
      ...
      shape.forEach((m, k1) => (
        m.forEach((n, k2) => {
          if (n && xy.get(0) + k1 >= 0) { // 竖坐标可以为负
            const posX = xy.get(0) + k1;
            const posY = xy.get(1) + k2;
            
            // Проверяем, что позиция в пределах поля (для всех режимов)
            if (posY >= 0 && posY < 10 && posX >= 0 && posX < 20) {
              let line = matrix.get(posX);
              let color;
              if (line.get(posY) === 1 && !clearLines) { // 矩阵与方块重合
                color = 2;
              } else {
                color = 1;
              }
              line = line.set(posY, color);
              matrix = matrix.set(posX, line);
            }
          }
        })
      ));
