...
// ============================================================
// 0. Определение платформы и параметров запуска
// ============================================================

// Функция для получения параметров запуска из URL
...
const platform = launchParams.vk_client === 'ok' ? 'ok' : 'vk';
const userId = parseInt(launchParams.viewer_id || launchParams.user_id || '0');

console.log(`🔍 Платформа: ${platform}, ID пользователя: ${userId}`);
// ============================================================
...
class AdManager {
    ...
    this.bannerPending = false;
     this.platform = platform; // 'vk' или 'ok'
        this.userId = userId;
        // Префикс для ключей хранилища (VK Storage и localStorage)
        this.storagePrefix = this.platform === 'ok' ? 'ok_' : '';
    }
   ...
   showRewardedAd() {
    return new Promise((resolve) => {
        ...
        const timeoutId = setTimeout(() => {
            console.warn('⏰ Таймаут показа рекламы (120 сек)');
            resolve(false);
        }, 120000);
        ...
        .then((result) => {
            ...
            // Проверка вложенного поля data
            if (result.data) {
                const data = result.data;
                if (data.result === true || data.success === true || data.status === 'success') {
                    resolve(true);
                    return;
                }
            }

            // Если нет явной ошибки и результат не равен false – считаем успехом
            if (result.error === undefined && result.result !== false) {
                resolve(true);
                return;
            }
...
this.vkUserId = null;
        ...
        this.cells = [];                // массив для хранения DOM-элементов ячеек
        this.gridHandler = null;
this.buildGrid();               // создаём сетку один раз
        this.numPanel = document.getElementById('num-panel');
        this.numPanelBuilt = false; 
        this.renderNumPanel(); 
...
document.getElementById('dailyBonusUnavailableModal').addEventListener('pointerdown', (e) => { e.preventDefault();
    if (e.target === e.currentTarget) {
        ...
        this.initAdBanner();
// Если платформа — Одноклассники, скрываем кнопки "Пригласить друзей" и "Таблица лидеров"
if (this.platform === 'ok') {
    const inviteBtn = document.getElementById('btnInviteFriends');
    const leaderBtn = document.getElementById('btnLeaderboard');
...
async initVK() {
    ...
    try {
        // Если ID пользователя уже получен из параметров запуска
        if (this.userId) {
            this.vkUserId = this.userId;
            this.vkInitialized = true;
            console.log(`👤 Пользователь ${this.platform} (из launch params):`, this.vkUserId);
        ...
        // Fallback: пытаемся получить через VKWebAppGetUserInfo
        ...
        // fallback – пробуем получить viewer_id из URL
...
loadDiamonds() {
    ...
    if (!this.userId || !this.vkInitialized) {
        this.loadDiamondsLocal();
        return Promise.resolve();
    }

    const keys = [
        `${this.storagePrefix}diamonds`,
        `${this.storagePrefix}bonusDate`
    ];
...
loadDiamondsLocal() {
    try {
        const saved = localStorage.getItem(`${this.storagePrefix}sudoku_diamonds`);
        this.currentDiamonds = saved ? parseInt(saved) : 0;
        const bonusDate = localStorage.getItem(`${this.storagePrefix}sudoku_bonusDate`);
...
saveDiamonds() {
    // Сохраняем локально
    try {
        localStorage.setItem(`${this.storagePrefix}sudoku_diamonds`, String(this.currentDiamonds));
        if (this.lastBonusDate) {
            localStorage.setItem(`${this.storagePrefix}sudoku_bonusDate`, this.lastBonusDate);
        }
    } catch (e) {}
    ...
    if (this.userId && this.vkInitialized) {
        vkBridge.send('VKWebAppStorageSet', {
            key: `${this.storagePrefix}diamonds`,
        ...
        if (this.lastBonusDate) {
            vkBridge.send('VKWebAppStorageSet', {
                key: `${this.storagePrefix}bonusDate`,
...
showLeaderboard() {
    if (this.platform === 'ok') {
        this.showGlobalToast('❌ Таблица лидеров недоступна в Одноклассниках', true);
        return;
    }
    this.syncLeaderboard();
    ...
    .catch((err) => {
        ...
        this.showGlobalToast('❌ Не удалось открыть таблицу лидеров', true);
    });
}
...
// ============================================================
syncLeaderboard() {
     // Для OK таблица лидеров не поддерживается
    if (this.platform === 'ok') return Promise.resolve();
...
inviteFriends() {
    this.sound.click();
    if (this.platform === 'ok') {
        this.showGlobalToast('❌ Приглашение друзей недоступно в Одноклассниках', true);
        return;
    }
    const sendMethod = vkBridge.sendPromise || vkBridge.send;
...
loadAchievements() {
    ...
    try {
        const local = localStorage.getItem(`${this.storagePrefix}sudoku_achievements`);
        if (local) {
        ...
        // Загружаем счётчики с префиксом
        const games = localStorage.getItem(`${this.storagePrefix}sudoku_totalGames`);
        if (games) this.totalGames = parseInt(games) || 0;
        const winStreak = localStorage.getItem(`${this.storagePrefix}sudoku_winStreak`);
        if (winStreak) this.winStreak = parseInt(winStreak) || 0;
        const perfectWins = localStorage.getItem(`${this.storagePrefix}sudoku_perfectWins`);
        const winsByLevel = localStorage.getItem(`${this.storagePrefix}sudoku_winsByLevel`);
        if (winsByLevel) { try { this.winsByLevel = JSON.parse(winsByLevel); } catch(e) {} }
        const noHintWins = localStorage.getItem(`${this.storagePrefix}sudoku_noHintWins`);
        if (noHintWins) this.noHintWins = parseInt(noHintWins) || 0;
        const totalChecks = localStorage.getItem(`${this.storagePrefix}sudoku_totalChecks`);
        if (totalChecks) this.totalChecks = parseInt(totalChecks) || 0;
        const totalPlaced = localStorage.getItem(`${this.storagePrefix}sudoku_totalPlaced`);
        if (totalPlaced) this.totalPlaced = parseInt(totalPlaced) || 0;
        const totalGameTime = localStorage.getItem(`${this.storagePrefix}sudoku_totalGameTime`);
        if (totalGameTime) this.totalGameTime = parseInt(totalGameTime) || 0;
        const wins = localStorage.getItem(`${this.storagePrefix}sudoku_totalWins`);
        if (wins) this.totalWins = parseInt(wins) || 0;
        const hints = localStorage.getItem(`${this.storagePrefix}sudoku_totalHints`);
        if (hints) this.totalHintsUsed = parseInt(hints) || 0;
        const bonuses = localStorage.getItem(`${this.storagePrefix}sudoku_totalBonuses`);
        if (bonuses) this.totalBonuses = parseInt(bonuses) || 0;
        const maxWinStreak = localStorage.getItem(`${this.storagePrefix}sudoku_maxWinStreak`);
    ...
    // Загружаем из VK Storage с префиксом
    const keys = [
        `${this.storagePrefix}achievements`,
        `${this.storagePrefix}totalWins`,
        `${this.storagePrefix}totalHints`,
        `${this.storagePrefix}totalBonuses`,
        `${this.storagePrefix}winStreak`,
        `${this.storagePrefix}maxWinStreak`,
        `${this.storagePrefix}winsByLevel`,
        `${this.storagePrefix}noHintWins`,
        `${this.storagePrefix}perfectWins`,
        `${this.storagePrefix}totalChecks`,
        `${this.storagePrefix}totalPlaced`,
        `${this.storagePrefix}totalGameTime`,
        `${this.storagePrefix}totalGames`
    ];
    ...
    return vkBridge.send('VKWebAppStorageGet', { keys })
        .then((data) => {
            if (data && data.keys) {
                data.keys.forEach(item => {
                    const key = item.key.replace(this.storagePrefix, '');
                    if (key === 'achievements') {
                        try {
...
// ============================================================
saveAchievements() {
    // Сохраняем в localStorage с префиксом
    try {
        localStorage.setItem(`${this.storagePrefix}sudoku_achievements`, JSON.stringify(this.achievements));
        localStorage.setItem(`${this.storagePrefix}sudoku_totalWins`, String(this.totalWins));
        localStorage.setItem(`${this.storagePrefix}sudoku_totalHints`, String(this.totalHintsUsed));
        localStorage.setItem(`${this.storagePrefix}sudoku_totalBonuses`, String(this.totalBonuses));
        localStorage.setItem(`${this.storagePrefix}sudoku_winStreak`, String(this.winStreak));
        localStorage.setItem(`${this.storagePrefix}sudoku_maxWinStreak`, String(this.maxWinStreak));
        localStorage.setItem(`${this.storagePrefix}sudoku_winsByLevel`, JSON.stringify(this.winsByLevel));
        localStorage.setItem(`${this.storagePrefix}sudoku_noHintWins`, String(this.noHintWins));
        localStorage.setItem(`${this.storagePrefix}sudoku_perfectWins`, JSON.stringify(this.perfectWins));
        localStorage.setItem(`${this.storagePrefix}sudoku_totalChecks`, String(this.totalChecks));
        localStorage.setItem(`${this.storagePrefix}sudoku_totalPlaced`, String(this.totalPlaced));
        localStorage.setItem(`${this.storagePrefix}sudoku_totalGames`, String(this.totalGames));
    } catch (e) {}

    // Сохраняем в VK Storage с префиксом
    if (this.userId && this.vkInitialized) {
        const sets = [
            { key: `${this.storagePrefix}achievements`, value: JSON.stringify(this.achievements) },
            { key: `${this.storagePrefix}totalWins`, value: String(this.totalWins) },
            { key: `${this.storagePrefix}totalHints`, value: String(this.totalHintsUsed) },
            { key: `${this.storagePrefix}totalBonuses`, value: String(this.totalBonuses) },
            { key: `${this.storagePrefix}totalGames`, value: String(this.totalGames) },
            { key: `${this.storagePrefix}winStreak`, value: String(this.winStreak) },
            { key: `${this.storagePrefix}maxWinStreak`, value: String(this.maxWinStreak) },
            { key: `${this.storagePrefix}winsByLevel`, value: JSON.stringify(this.winsByLevel) },
            { key: `${this.storagePrefix}noHintWins`, value: String(this.noHintWins) },
            { key: `${this.storagePrefix}perfectWins`, value: JSON.stringify(this.perfectWins) },
            { key: `${this.storagePrefix}totalChecks`, value: String(this.totalChecks) },
            { key: `${this.storagePrefix}totalPlaced`, value: String(this.totalPlaced) },
            { key: `${this.storagePrefix}totalGameTime`, value: String(this.totalGameTime) }
        ];

        sets.forEach(({ key, value }) => {
            vkBridge.send('VKWebAppStorageSet', { key, value })
                .catch(err => console.warn(`Ошибка сохранения ${key}:`, err));
        });
    }
}
...
showToast(message, isError = false) {
    ...
    buildGrid() {
    this.gridElement.innerHTML = '';   // очищаем
    this.cells = [];
    ...
    // Один обработчик на всю сетку (делегирование)
    // Удаляем предыдущий, если был (чтобы не навешивать несколько)
    // Просто заменим: уберём все обработчики, добавим новый
    // Можно использовать removeEventListener, но проще заменить элемент (но мы не хотим пересоздавать сетку)
    // Поэтому сохраним обработчик в свойство, чтобы потом удалить старый, если он есть.
    if (this.gridHandler) {
        this.gridElement.removeEventListener('pointerdown', this.gridHandler);
    }
    this.gridHandler = (e) => {
    ...
    this.gridElement.addEventListener('pointerdown', this.gridHandler);
}
...
render() {
    ...
    for (let i = 0; i < 81; i++) {
        ...
        // Сбрасываем все классы, кроме базового
        cell.className = 'cell';
        // Добавляем границы блоков 3x3
        ...
        // Ошибки (только на лёгком уровне)
        if (this.difficulty === 'easy' && !this.given[r][c] && val !== 0 && val !== this.solution[r][c]) {
        ...
        if (this.checkedCells[cellKey] !== undefined) {
            if (this.checkedCells[cellKey] === true) {
                ...
                cell.classList.add('checked-error');
            }
        }
        ...
        // Подсветка строк/столбцов/блоков и одинаковых цифр (кроме эксперта)
        if (this.selectedRow !== -1 && this.selectedCol !== -1) {
            const selVal = this.grid[this.selectedRow][this.selectedCol];
            if (this.difficulty !== 'expert' && selVal !== 0 && val === selVal && !(r === this.selectedRow && c === this.selectedCol)) {
        ...
        // Дата-атрибуты уже установлены, их не меняем
    }

    // Обновляем статус (таймер и т.д.)
    this.updateStatus();
}
// ============================================================
...
renderNumPanel() {
    // Если панель уже создана – ничего не делаем
    if (this.numPanelBuilt) return;
    ...
    // Отмечаем, что панель создана
    this.numPanelBuilt = true;
}
