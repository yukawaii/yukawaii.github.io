...
const CATEGORIES = {
    ...
    food: {
        ...
        subcategories: {
            ...
            hard: { name: 'Сложнее', folder: 'food/hard', count: 54 }
        }
    },
    items: {
        name: '📦 Предметы',
...
// ===== НОВАЯ ИНИЦИАЛИЗАЦИЯ (как в Судоку) =====
...
async function initApp() {
    ...
    try {
        // 1. Инициализируем VK (как в Судоку)
        ...
        // 2. Загружаем данные (локальные + VK Storage)
        await loadAllData();
        
        // 3. Предзагружаем рекламу (как в Судоку)
        if (typeof preloadRewardAd === 'function') {
            preloadRewardAd(); // не ждем, просто запускаем
        }
        ...
        if (loading) loading.style.display = 'none';
        // Показываем тост об ошибке, но приложение работает
        showToast('⚠️ Некоторые данные не загрузились, но игра работает');
    }
}
...
async function loadAllData() {
    ...
    // Загружаем из VK Storage (если доступно) — МЕРЖИМ, НЕ ПЕРЕЗАПИСЫВАЕМ
    if (typeof loadAppStateFromVK === 'function') {
        try {
            await loadAppStateFromVK();
            console.log('✅ Данные из VK Storage загружены');
        } catch(e) {
            console.warn('⚠️ Ошибка загрузки из VK Storage:', e);
        }
    }
...
function openUnlockModal(categoryKey, levelIndex, subKey) {
    // ✅ Очищаем флаги кисти, чтобы они не мешали
...
function preloadRewardAd() {
    if (typeof vkBridge === 'undefined' || !vkBridge.send) {
        console.log('ℹ️ VK Bridge не доступен для предзагрузки рекламы');
        return;
    }
    ...
    .then(function(data) {
        if (data && data.result) {
            ...
            console.log('ℹ️ Реклама не найдена для предзагрузки');
        }
    })
...
function watchAdForUnlock() {
    if (!pendingUnlockLevel) {
        showToast('❌ Нет уровня для открытия');
        return;
    }
    ...
    let timeoutId = setTimeout(() => {
        ...
        showToast('⏰ Превышено время ожидания рекламы');
        console.warn('⏰ Таймаут ожидания рекламы для уровня');
    }, 65000); // 65 секунд (чуть больше, чем в showRewardedAd)
    ...
    showRewardedAd()
        .then((success) => {
            ...
            document.getElementById('adLoadingModal').classList.remove('show');
            if (success) {
                console.log('✅ Реклама просмотрена, открываем уровень');
                unlockAndOpenLevelWithData(unlockData);
            } else {
                showAdError();
            }
...
function watchAdForBrushUnlock() {
    ...
    if (!brushId) {
        console.warn('⚠️ Нет ID кисти для разблокировки');
        showToast('❌ Ошибка: кисть не выбрана');
        return;
    }
    
    console.log('🎬 Запрос разблокировки кисти:', brushId);
    closeUnlockModal();
    document.getElementById('adLoadingModal').classList.add('show');
    ...
    let timeoutId = setTimeout(() => {
        document.getElementById('adLoadingModal').classList.remove('show');
        showToast('⏰ Превышено время ожидания рекламы, кисть открыта!');
        console.warn('⏰ Таймаут ожидания рекламы для кисти');
        
        // Всё равно разблокируем кисть
        if (!brushesState[brushId]) {
            brushesState[brushId] = { unlocked: false, expiresAt: null };
        }
        brushesState[brushId].unlocked = true;
        brushesState[brushId].expiresAt = Date.now() + (24 * 60 * 60 * 1000);
    ...
    if (!bridge || !bridge.send || !navigator.onLine) {
        clearTimeout(timeoutId);
        document.getElementById('adLoadingModal').classList.remove('show');
        // Разблокируем без рекламы
        if (!brushesState[brushId]) {
            brushesState[brushId] = { unlocked: false, expiresAt: null };
        }
        brushesState[brushId].unlocked = true;
    ...
    bridge.send('VKWebAppCheckNativeAds', { ad_format: 'reward' })
        .then((checkData) => {
            if (!checkData || !checkData.result) {
                ...
                window._pendingBrushUnlockFromColoring = false;
                window.dispatchEvent(new CustomEvent('brushUnlocked', { detail: { brushId } }));
                showToast('⚠️ Реклама недоступна, но кисть открыта!');
                return;
            }
        ...
        .then((showData) => {
            ...
            if (showData && (showData.result === true || showData.success === true)) {
                ...
                // Разблокируем даже если реклама не показана
                if (!brushesState[brushId]) {
                    brushesState[brushId] = { unlocked: false, expiresAt: null };
                }
                brushesState[brushId].unlocked = true;
                ...
                window._pendingBrushUnlockFromColoring = false;
                window.dispatchEvent(new CustomEvent('brushUnlocked', { detail: { brushId } }));
                showToast('⚠️ Реклама не показана, но кисть открыта!');
            }
        })
        .catch((error) => {
            clearTimeout(timeoutId);
            document.getElementById('adLoadingModal').classList.remove('show');
            console.warn('⚠️ Ошибка рекламы для кисти:', error);
...
// ===== ОБНОВЛЕНИЕ МЕНЮ КИСТЕЙ ПОСЛЕ РАЗБЛОКИРОВКИ =====
function updateBrushMenuAfterUnlock(brushId) {
    console.log('🔄 Обновление меню кистей после разблокировки:', brushId);
    
    // Ищем элемент jl-coloringbook на странице
    const coloringBook = document.querySelector('jl-coloringbook');
    if (coloringBook && typeof coloringBook.refreshBrushMenu === 'function') {
        coloringBook.refreshBrushMenu();
    } else {
        // Если метод не найден, просто закрываем и открываем меню через событие
        window.dispatchEvent(new CustomEvent('brushUnlocked', { detail: { brushId } }));
    }
}
// ===== РАСКРАСКА =====
...
coloringBook.style.userSelect = 'none';
  ...
  const pointsDisplay = document.getElementById('coloringPoints');
if (pointsDisplay) {
    pointsDisplay.textContent = appState.totalPoints || 0;
}
...
setTimeout(() => {
    ...
    coloringBook.addEventListener('progressUpdate', function(e) {
        ...
        if (percent >= 70 && !pointsEarned) {
            pointsEarned = true;
            const points = 1;
            document.getElementById('coloringPoints').textContent = appState.totalPoints;
            addPoints(points);
...
function isBrushUnlocked(brushId) {
    ...
    console.log(`🔍 Проверка кисти ${brushId}:`, state);
...
function saveAppStateToVK() {
    ...
    console.log('📤 Сохранение в VK Storage:', {
        points: data.totalPoints,
        colored: data.coloredImages.length,
...
function mergeDataFromVK(data) {
    ...
    // unlockedAchievements — объединяем
...
function loadAppStateFromVK() {
    const bridge = getVKBridge ? getVKBridge() : null;
    if (!bridge) {
        console.log('ℹ️ VK Bridge не доступен, пропускаем загрузку');
                ...
                if (value) {
                    try {
                        ...
                        console.log('📥 Данные из VK Storage получены:', {
                            points: data.totalPoints,
                            colored: data.coloredImages?.length || 0,
                            achievements: data.unlockedAchievements?.length || 0
                        });
                        
                        // ===== ВОТ ЗДЕСЬ ВСТАВЛЯЕМ ВЫЗОВ =====
                        mergeDataFromVK(data);
                        // ====================================
                        
                        return data;
                    } catch(e) {
                        console.warn('⚠️ Ошибка парсинга данных из VK:', e);
                    }
                } else {
    ...
    return new Promise((resolve, reject) => {
        ...
        const timeoutId = setTimeout(() => {
            if (!resolved) {
                ...
                console.warn('⏰ Таймаут загрузки рекламы (60 сек)');
                reject('Ad loading timeout');
            }
        }, 60000); // 60 секунд
...
function watchAdForDailyBonus() {
    ...
    let timeoutId = setTimeout(() => {
        document.getElementById('adLoadingModal').classList.remove('show');
        showToast('⏰ Превышено время ожидания, бонус получен!');
        dailyBonusData.lastClaimDate = new Date().toISOString().slice(0,10);
        saveDailyBonusData();
        appState.totalPoints += 5;
        updateStats();
...
(function forceHideSplash() {
    setTimeout(function() {
        ...
        if (splash && splash.style.display !== 'none') {
            splash.style.display = 'none';
            console.log('✅ Экран загрузки скрыт принудительно (таймаут 30с)');
        }
    }, 30000);
})();