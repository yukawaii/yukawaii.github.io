(function () {
	...
	var Player = Mario.Player = function (pos) {
		...
		Mario.Entity.call(this, {
			pos: pos,
			sprite: new Mario.Sprite('mario_assets/sprites/player.png', [80, 32], [16, 16], 0),
			hitbox: [0, 0, 16, 16]
		});
		this.moveAcc = 0.4; // Responsive acceleration
	};

	Mario.Util.inherits(Player, Mario.Entity);

	Player.prototype.run = function () {
		this.maxSpeed = 1.5; // Sprinting disabled (was 2.5)
	...
	Player.prototype.noWalk = function () {
		this.maxSpeed = 0;
		this.acc[0] = 0; // CRITICAL: Stop accelerating when keys are released
	...
	Player.prototype.jump = function () {
		...
		if (this.jumping) {
			...
			this.canJump = false;
			this.standing = false;
			this.vel[1] = -7; // Faster jump impulse
			if (this.power === 0) {
	...
	Player.prototype.setAnimation = function () {
		...
		if (this.jumping) {
			...
			if (this.shooting) {
				this.sprite.pos[0] += 160;
				this.shooting -= Mario.dt * 60;
				if (this.shooting <= 0) this.shooting = 0;
			}
		}
		...
		if (this.left) {
			this.sprite.img = 'mario_assets/sprites/playerl.png';
		} else {
			this.sprite.img = 'mario_assets/sprites/player.png';
		}
	};
	...
	Player.prototype.update = function (dt, vX) {
		...
		if (this.invincibility) {
			this.invincibility -= dt * 60;
			if (this.invincibility <= 0) this.invincibility = 0;
		}
		...
		if (Math.abs(this.vel[0]) > this.maxSpeed) {
			var brakePower = 0.8;
			if (Math.abs(this.vel[0]) < brakePower) {
				this.vel[0] = 0;
			} else {
				this.vel[0] -= brakePower * (this.vel[0] / Math.abs(this.vel[0]));
			}
			this.acc[0] = 0;
		}
		...
		if (this.dying) {
			...
			this.dying -= dt * 60;
			if (this.dying <= 0) {
				// Submit Scores on Death
				if (window.submitMarioScores) {
					var startX = level.startX !== undefined ? level.startX : (level.playerPos ? level.playerPos[0] : 0);
					var endX = level.endX !== undefined ? level.endX : (level.exit * 16);
					var progress = Math.max(0, Math.min(100, Math.floor(((this.pos[0] - startX) / (endX - startX)) * 100)));
					window.submitMarioScores(progress, this.coins, 0); // 0 time means don't submit time
				}
				...
				vX = 0;
				gameStarted = false;
				if (window.resetAutoRun) window.resetAutoRun();
			}
		}
		else {
			this.acc[1] = 0.35; // Increased gravity for snappier jump
			if (this.pos[1] > 240) {
				this.die();
			}
		}
		...
		this.vel[0] += this.acc[0] * dt * 60;
		this.vel[1] += this.acc[1] * dt * 60;
		this.pos[0] += this.vel[0] * dt * 60;
		this.pos[1] += this.vel[1] * dt * 60;
