...
function showText(text) {
    var messages = {
        ...
        'pausedAndroid': "<div class='centeredHeader unselectable'>Пауза</div>",
        'pausediOS': "<div class='centeredHeader unselectable'>Пауза</div>",
        'pausedOther': "<div class='centeredHeader unselectable'>Пауза</div>",
        'start': "<div class='centeredHeader unselectable' style='line-height:80px;'>Нажмите, чтобы начать</div>"
    };
    if (text == 'paused') {
...
function renderBeginningText() {
    ...
    if(/mobile|Mobile|iOS|Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent)) {
        mob = true;
        input_text = 'Нажимайте на экран слева и справа,'
        action_text = 'чтобы вращать Хитрекс.'
        score_text = 'Соедините 3+ блоков и получите очки!'
        fontSize = 35
    } else {
        mob = false
        input_text = 'Используйте клавиши-стрелки,'
        action_text = 'чтобы вращать Хитрекс.'
        score_text = 'Соедините 3+ блоков и получите очки!'
        fontSize = 27
    }
