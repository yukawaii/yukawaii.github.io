...
levelupSound.volume = 0.3   // было 0.1
 ...
 isGameStarted = false;
isGameOver = false;
// Массив миссий: [порог_очков, id_миссии]
const missions = [
    { score: 100,   missionId: 3 },
    { score: 300,   missionId: 4 },
    { score: 500,   missionId: 5 },
    { score: 666,   missionId: 6 },
    { score: 777,   missionId: 7 },
    { score: 1000,  missionId: 8 },
    { score: 1500,  missionId: 9 },
    { score: 2000,  missionId: 1 },
    { score: 3000,  missionId: 11 },
    { score: 5000,  missionId: 12},
    { score: 10000, missionId: 13 }
];

// Функция отправки миссии в VK
async function sendMissionToVK(userId, missionId, scoreValue) {
    return new Promise((resolve, reject) => {
        vkBridge.send('VKWebAppCallAPIMethod', {
            method: 'secure.addAppEvent',
            request_id: `mission_${missionId}_${Date.now()}`,
            params: {        
                user_id: userId,
        ...
        .then(data => {
            console.log(`🏆 Миссия ${missionId} (${scoreValue} очков) отправлена!`, data);
            resolve(data);
        })
        .catch(error => {
            console.error(`❌ Ошибка отправки миссии ${missionId}:`, error);
            reject(error);
        });
    });
}

// Функция проверки и отправки миссий
async function checkAndSendAchievements(score) {
    ...
    for (const mission of missions) {
        if (score >= mission.score) {
            const key = `mission_${mission.missionId}`;
            const alreadySent = localStorage.getItem(key);            
            if (!alreadySent) {
                try {
                    await sendMissionToVK(userId, mission.missionId, mission.score);
                    localStorage.setItem(key, '1');
                } catch(e) {
                    console.log(`Не удалось отправить миссию ${mission.missionId}`);
                }
            }
        }
    }
